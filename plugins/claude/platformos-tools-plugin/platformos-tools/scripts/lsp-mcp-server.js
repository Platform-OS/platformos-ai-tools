...
const LOG_FILE = join(ROOT, 'platformos-tools.log');

function log(msg) {
  appendFile(LOG_FILE, `[${new Date().toISOString()}] [platformos-tools] ${msg}\n`).catch(() => {});
}
...
class PlatformOSLSPClient {
  ...
  async initialize(rootUri) {
    await this.#req('initialize', {
      ...
      clientInfo: { name: 'claude-platformos-tools', version: '1.0.0' },
      rootUri,
      capabilities: {
        textDocument: {
          publishDiagnostics: {},
...
const TOOLS = [
  {
    name: 'platformos_diagnostics',
    description:
      'Run platformOS pos-cli check (linter/validator) on a Liquid or GraphQL file ' +
...
async function callTool(lsp, ready, name, args) {
  switch (name) {
    case 'platformos_diagnostics': {
      const absPath = toAbs(args.file_path);
      if (!absPath.endsWith('.liquid') && !absPath.endsWith('.graphql'))
        return `platformos_diagnostics only supports .liquid and .graphql files. Got: ${args.file_path}`;
      log(`tool: diagnostics file=${args.file_path}`);
      const result = await runCheck(absPath);
      return result?.text ?? `No issues found in ${args.file_path} (pos-cli check passed)`;
    }

    case 'platformos_hover': {
      log(`tool: hover file=${args.file_path} line=${args.line} char=${args.character}`);
      await ready;
      if (!lsp.initialized) return 'platformOS LSP is not ready yet. Try again in a moment.';
      const absPath = toAbs(args.file_path);
...
async function main() {
  ...
  const lsp = new PlatformOSLSPClient().start('pos-cli', ['lsp']);
      ...
      switch (method) {
        case 'initialize':
          respond(id, {
            ...
            capabilities: { tools: {} },
            serverInfo: { name: 'platformos-tools', version: '1.0.0' },
          });
          break;
...
main().catch((err) => {
  process.stderr.write(`[platformos-tools] fatal: ${err.message}\n`);
  process.exit(1);
});
