...
Every value in Liquid has a type that determines how it behaves in output, comparisons, and iteration. platformOS extends standard Liquid with Hash and Array initialization via `parse_json`, and adds operators like `hash_assign` and `array_any` for richer data manipulation.
