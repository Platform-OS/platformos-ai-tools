---
name: pos-auth
description: Protocols for implementing user authentication in platformOS using the User Module
---

## Overview

This skill provides **absolute, non-negotiable protocols** for implementing user authentication in platformOS applications using the `pos-module-user`. YOU MUST follow every instruction precisely. Authentication is security-critical—deviations introduce vulnerabilities that compromise user data and system integrity.

The platformOS User Module provides pre-built authentication infrastructure including:
...
platformOS provides these security features automatically:
...
**CRITICAL RULES:**
- Password is automatically hashed by platformOS - NEVER hash it yourself
...
pos-cli logs staging &
pos-cli test run staging test/auth
```
