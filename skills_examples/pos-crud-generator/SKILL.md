...
```bash
pos-cli generate run modules/core/generators/crud <resource> <properties> --include-views
```

### Generate CRUD without views
```bash
pos-cli generate run modules/core/generators/crud <resource> <properties>
```

### Get help on generator
```bash
pos-cli generate run modules/core/generators/crud --help
```
...
### Blog Article
```bash
pos-cli generate run modules/core/generators/crud article title:string body:text published_at:datetime author_id:string --include-views
```

### E-commerce Product
```bash
pos-cli generate run modules/core/generators/crud product name:string description:text price:float sku:string in_stock:boolean image:upload --include-views
```

### User Profile
```bash
pos-cli generate run modules/core/generators/crud profile first_name:string last_name:string bio:text avatar:upload user_id:string --include-views
```
